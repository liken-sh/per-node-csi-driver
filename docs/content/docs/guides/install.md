...
creates the `CSIDriver` object, the `StorageClass` named `per-node`,
the `ServiceAccount` and its role, and the `DaemonSet` that runs the
node plugin beside the kubelet's registrar on every node. There is no
...
node appears there after the registrar told its kubelet about the
plugin and the kubelet called it. A node missing from the answer has no
...
| `--metrics` | `:9808` | Where the node plugin serves its Prometheus gauge. Empty serves none. |
